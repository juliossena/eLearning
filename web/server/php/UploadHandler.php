    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
        ...
        if ($idCourse > 0) {
            ...
            $this->options = array(
                ...
                'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')).'/../../../dados/course_'.$idCourse.'/',
                'upload_url' => 'index.php?site=coursesInstructor&subSite=downloadFileCourse&idCourse='.$idCourse.'&idFile=',
                'user_dirs' => false,
                'mkdir_mode' => 0755,
                'param_name' => 'files',
...
//        return $this->options['upload_url'].$this->get_user_path();
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
                ...
                if ($append_file) {
                    ...
                    $fileEnviar = new Files();
                    $fileEnviar->setName($file->name);
                    $fileEnviar->setLocation('course_' . $idCourse . '/' . $file->name);
                    
                    $nameFile = explode(".", $file->name);
                    if ($nameFile[count($nameFile) - 1] == "jpg" || $nameFile[count($nameFile) - 1] == "gif" || $nameFile[count($nameFile) - 1] == "png" || $nameFile[count($nameFile) - 1] == "jpeg") {
                        $fileEnviar->setThumbnail('course_' . $idCourse . '/thumbnail/' . $file->name);
                    }
                    
                    $files = new ArrayObject();
                    $files->append($fileEnviar);
                    
                    $course->setFiles($files);
                    
                    $courseDAO = new CoursesDAO();
                    $courseDAO->insertFilesDAO($course, $file->name);
    ...
    public function delete($print_response = true) {
        ...
        foreach($file_names as $file_name) {
            ...
            if ($coursesDao->dropFiles($course)) {
